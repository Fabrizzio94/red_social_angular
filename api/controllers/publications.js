...
function probando (req, res) {
    res.status(200).send({message: 'Hola desde C publicacion'});
}
...
function deletePublication(req, res) {
    ...
    Publication.findOneAndRemove({'user': req.user.sub, '_id': publicationId }, (err, publicationRemoved) => {
        ...
        if(!publicationRemoved) return res.status(404).send({message:'No se ha encontrado la publicacion'});
        return res.status(200).send({message: 'La publicacion se ha borrado con exito'});
        ...
        if(file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif'){
            Publication.findOne({'user': req.user.sub , '_id': publicationId }).exec((err, publication) => {
                ...
                if(publication) {
                    ...
                    return removeFileOfUploads(res, file_path, 'No tienes permiso para actualizar esta publicacion!');
                }
            });
